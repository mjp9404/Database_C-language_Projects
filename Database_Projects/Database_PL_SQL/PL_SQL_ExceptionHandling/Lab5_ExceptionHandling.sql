...
-- Constant variables
  ...
  k_manager_job CONSTANT VARCHAR2(15) := 'MANAGER';
...
-- Check if an employee has a manger --
     SELECT COUNT(job), deptno
     INTO v_job_count, v_depno
     FROM emp
     WHERE job = k_manager_job
     GROUP BY deptno;
      
     IF v_job_count < 1 THEN
     RAISE ex_job_position;
     END IF;
     
-- Exception --
  EXCEPTION
  WHEN ex_job_position THEN
  DBMS_OUTPUT.PUT_LINE('Error occurs: department has to include with Manager');
  WHEN OTHERS THEN 
  ROLLBACK;  
  END;
  -------------------------------------------------------------------------------------------
